...
export const audioRecsSlice = createSlice({
    ...
    reducers: {
        ...
        removeRec: (state, action) => {
            const index = state.values.findIndex(element => element.id = action.payload)
...
export async function fetchAudioRecs(tags = [], title = "", startDate = "", endDate = "") {
...
export const { addRec, removeRec, updateRecList, clearAudioRecs, findIds } = audioRecsSlice.actions
